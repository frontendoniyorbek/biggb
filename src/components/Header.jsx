...
const Header = () => {
	...
	// --- Yangi kod: oynani kattalashtirganda open ni false qilamiz ---
	useEffect(() => {
		const handleResize = () => {
			// md: breakpoint Tailwind'da 768px
		...
		// open dependency kerak emas, faqat resize
	}, [open]);
	// ---------------------------------------------------------------
	...
	return (
		<header className='relative z-50 w-full bg-white py-5 lg:py-[28px]'>
			<div className='mx-auto flex justify-between items-center  px-5' style={{ maxWidth: '1523px' }}>
				{/* Logo, always links to home */}
				<NavLink to='/' className='font-bold text-lg lg:text-2xl tracking-widest flex items-center select-none'>
				...
				<div className='flex flex-col gap-4 px-8 mt-10 '>
					{navLinks.map((link, i) => (
						<NavLink
							...
							style={{
								animation: open ? `slideIn 0.6s cubic-bezier(.23,1.1,.32,1) forwards ${0.08 * i + 0.08}s` : 'none',
							}}
							onClick={() => setOpen(false)}>
							{link.name}
						</NavLink>
